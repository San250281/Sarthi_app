...
import com.example.speech.SpeechSpeechRecognizer
import com.example.speech.SpeechTextToSpeech
import com.example.speech.MediaAudioRecorder
...
class SaarthiViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun secureSignUp(name: String, email: String, passcode: String, onSuccess: () -> Unit, onError: (String) -> Unit) {
        ...
        viewModelScope.launch(Dispatchers.IO) {
            try {
                val hashed = com.example.util.SecurityUtils.hashPassword(passcode)
